...
@section('pageheader-title', 'Ebooks')
@section('breadcrumb')
			...
			@foreach($ebooks as $key=>$ebook)
		        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
		            <div class="card">
		                <div class="card-body">
		                    <div class="media">
		                        <img class="mr-3 user-avatar-lg rounded" src="{{url('public/ebook-thumbnails')}}/{{$ebook->ebook_thumbnail}}">
		                        <div class="media-body">
		                            <h3><?php echo strip_tags($ebook->ebook_title, '<br>') ?></h3>
		                            <p>{{$ebook->ebook_description}}</p>
		                        </div>
		                    </div>
		                    <div class="action-btns text-right">
	                        	<form action="{{route('ebooks.destroy',$ebook->id)}}" method="POST" id="form-{{$key}}">@csrf
	                        		<a href="{{route('ebooks.edit',$ebook->id)}}" class="btn btn-primary mr-1"><i class="fa fa-edit mr-2"></i>Edit</a>
								    <input type="hidden" name="_method" value="DELETE" />
								    <button type="button" data-toggle="modal" data-target="#exampleModal" class="btn btn-danger ebook-delete-btn"><i class="fa fa-trash mr-2"></i>Delete</button>
								</form>
	                        </div>
		                </div>
		            </div>
...
<script type="text/javascript">
    ...
    //
    $(document).delegate(".ebook-delete-btn","click",function(){
